...
public class SourceOperator<OUT, SplitT extends SourceSplit> extends AbstractStreamOperator<OUT>
    ...
    public void initReader() throws Exception {
        ...
        sourceReader = readerFactory.apply(context);//调用MySqlSource的createReader方法
    }

    @Override
    public void open() throws Exception {
        initReader();//初始化,调用MySqlSource的createReader方法===>创建SourceReader
        ...
        sourceReader.start();  // [再次分析：会调用MySqlSourceEnumerator的handleSplitRequest()方法，对切片请求进行处理]  ###对数据进行切片
        //MySqlSourceReader接收到切片分配请求后 调用SourceOperator的handleOperatorEvent()方法处理分配的切片   ###对数据进行修正
        eventTimeLogic.startPeriodicWatermarkEmits();//SourceOperator 初始化完毕后，调用emitNext()方法,进行数据下发
    }//MySqlSourceReader处理完一个全量切片后，会向MySqlSourceEnumerator发送已完成的切片信息，包含切片ID、HighWatermar 就调用MySqlSourceReader#onSplitFinished方法
    //全量阶段所有分片读取完毕后，MySqlHybridSplitAssigner 会创建BinlogSplit 进行后续增量读取 会调用MySqlSourceEnumerator的handleSplitRequest()方法再调用MySqlHybridSplitAssigner#createBinlogSplit方法
    @Override
    ...
    public InputStatus emitNext(DataOutput<OUT> output) throws Exception {
        ...
        return sourceReader.pollNext(currentMainOutput);//调用SourceReaderBase的pollNext()方法，将数据发送给MySqlRecordEmitter
    }
    ...
    public void handleOperatorEvent(OperatorEvent event) {
        if (event instanceof AddSplitEvent) {
            try { //sourceReader获取到了切片分配请求，那么就可以开始处理切片数据了.......
