...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>> extends AbstractInvokable
    ...
    void executeRestore() throws Exception {
        ...
        // task specific initialization 任务特定的初始化 比如：flink cdc时，在这里调用SourceOperatorStreamTask的init方法
        init();
        ...
        // executed before all operators are opened 调用SourceOperator的open()方法
        CompletableFuture<Void> allGatesRecoveredFuture = actionExecutor.call(this::restoreGates);//堆外内存资源申请 Register And Start the reader to the coordinator
