...
public class SplitFetcher<E, SplitT extends SourceSplit> implements Runnable {
    ...
    void runOnce() {
        try {
            ...
            // change.
            if (shouldRunFetchTask()) {//首次执行 AddSplitsTask 线程添加分片，以后执行 FetchTask 线程拉取数据
                runningTask = fetchTask;//FetchTask
            } else {
                runningTask = taskQueue.take();//AddSplitsTask
            }
            ...
            LOG.debug("Prepare to run {}", runningTask);//AddSplitsTask：向切片队列中添加已分配的切片信息 || FetchTask：fetch()方法读取数据到事件队列
            if (!wakeUp.get() && runningTask.run()) {//调用对应Task的run方法【1，AddSplitsTask 调用 2，FetchTask】
