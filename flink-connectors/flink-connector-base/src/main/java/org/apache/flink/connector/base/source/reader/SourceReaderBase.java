...
public abstract class SourceReaderBase<E, T, SplitT extends SourceSplit, SplitStateT>
    ...
    @Override
    public InputStatus pollNext(ReaderOutput<T> output) throws Exception {//SourceReaderBase 处理切片数据流程
        ...
        if (recordsWithSplitId == null) {
            recordsWithSplitId = getNextFetch(output); //从elementsQueue 获取数据
            if (recordsWithSplitId == null) {
        ...
        while (true) {
            // Process one record.  通过MySqlRecords从迭代器中读取单条数据
            final E record = recordsWithSplitId.nextRecordFromSplit();
            if (record != null) {
                // emit the record. MySqlRecordEmitter 通过 RowDataDebeziumDeserializeSchema 将数据转换为Rowdata
