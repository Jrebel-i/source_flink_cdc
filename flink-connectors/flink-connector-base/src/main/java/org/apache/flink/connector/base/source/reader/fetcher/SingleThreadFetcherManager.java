...
public class SingleThreadFetcherManager<E, SplitT extends SourceSplit>
    ...
    public void addSplits(List<SplitT> splitsToAdd) {
        ...
        if (fetcher == null) {
            fetcher = createSplitFetcher();//创建一个 SplitFetcher线程
            // Add the splits to the fetchers.
            fetcher.addSplits(splitsToAdd);//向 taskQueue 添加、执行AddSplitsTask 任务
            startFetcher(fetcher);//执行 FetchTask, 使用Debezium API进行读取数据  启动执行SplitFetcher的run()方法
        } else {
