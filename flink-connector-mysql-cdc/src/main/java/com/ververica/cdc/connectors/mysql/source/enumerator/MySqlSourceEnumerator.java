...
public class MySqlSourceEnumerator implements SplitEnumerator<MySqlSplit, PendingSplitsState> {
    ...
    public MySqlSourceEnumerator(
        ...
        this.splitAssigner = splitAssigner;//传入的是MySqlHybridSplitAssigner或者MySqlBinlogSplitAssigner
        this.readersAwaitingSplit = new TreeSet<>();
    }

    @Override
    public void start() {//做初始化操作  如果为MySqlHybridSplitAssigner，就创建ChunkSplitter用来划分切片  筛选出要读的表名称
        splitAssigner.open();//使用 MySqlHybridSplitAssigner 对全量+增量数据进行切片
        this.context.callAsync(
                this::getRegisteredReader,
                this::syncWithReaders,//启动周期调度线程，要求 SourceReader 向 SourceEnumerator 发送已完成但未发送ACK事件的切片信息
