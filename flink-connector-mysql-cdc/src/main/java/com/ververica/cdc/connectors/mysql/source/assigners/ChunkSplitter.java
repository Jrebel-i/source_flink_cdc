...
class ChunkSplitter {
    ...
    public Collection<MySqlSnapshotSplit> generateSplits(TableId tableId) {
        try (JdbcConnection jdbc = openJdbcConnection(sourceConfig)) {
            ...
            try {
                chunks = splitTableIntoChunks(jdbc, tableId, splitColumn); //切分表为chunks 按主键列将数据划分成多个切片 根据物理主键划分切片
            } catch (SQLException e) {
                throw new FlinkRuntimeException("Failed to split chunks for table " + tableId, e);
            }

            // convert chunks into splits      ChunkRange 包装成MySqlSnapshotSplit
    ...
    private List<ChunkRange> splitTableIntoChunks(
        ...
        if (isEvenlySplitColumn(splitColumn)) {
            ...
            if (dataIsEvenlyDistributed) {//数据是均匀分配的
                ...
                return splitUnevenlySizedChunks(//按主键非均匀划分  通过连续计算下一个块最大值，将表拆分为大小不均匀的块
    ...
    private List<ChunkRange> splitUnevenlySizedChunks(
        ...
        while (chunkEnd != null && ObjectUtils.compare(chunkEnd, max) <= 0) {
            ...
            chunkStart = chunkEnd;
            chunkEnd = nextChunkEnd(jdbc, chunkEnd, tableId, splitColumnName, max, chunkSize); //计算下一个块最大值
        }
        // add the ending split
        splits.add(ChunkRange.of(chunkStart, null));
        return splits;
